...
# 업로드할 프로그램 파일의 정확한 이름 (이 이름으로만 업로드 및 다운로드됨)
PROGRAM_FILENAME = 'OverView_Client.zip'
ALLOWED_EXTENSIONS = {'zip'}

app = Flask(__name__)
app.config['UPLOAD_FOLDER'] = UPLOAD_FOLDER
app.config['SECRET_KEY'] = 'supersecretkey' # 실제 운영 시에는 더 복잡한 키로 변경하세요.

# ... 기존 app.py 코드의 윗부분은 그대로 둡니다 ...
...
<head>
    ...
    <style>
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .logo { font-size: 28px; font-weight: 700; color: var(--primary-neon); text-shadow: 0 0 5px var(--primary-neon), 0 0 10px var(--primary-neon); cursor: pointer; }
        ...
        .hero-content .highlight {
            display: block;
            font-size: 72px;
            color: #cce7ff; /* 텍스트 색상을 더 밝고 선명하게 */
            /* 그림자 효과를 더 부드럽고 은은하게 수정 */
            text-shadow: 
                0 0 5px rgba(100, 181, 246, 0.7),
                0 0 12px rgba(100, 181, 246, 0.5),
        ...
        <section id="download" class="section">
            <div class="container">
                <div class="download-box">
                    ...
                    <a href="{{ url_for('download_file', filename=program_filename) }}" class="btn">OverView 다운로드 (v1.0)</a>
                </div>
            </div>
        </section>
            ...
            <div class="container">
                ...
                <div class="grid">
                    <div class="card feature-card">
                        ...
                        <h3>실시간 화면 공유</h3>
                        <p>지연 시간을 최소화한 고화질 화면 스트리밍으로 여러 대의 PC를 동시에 모니터링하세요.</p>
                    </div>
                    <div class="card feature-card">
                        <div class="icon">🖱️</div>
                        <h3>원격 키보드/마우스</h3>
                        <p>내 PC를 조작하듯, 원격지 PC의 키보드와 마우스를 완벽하게 제어할 수 있습니다.</p>
                    </div>
                    <div class="card feature-card">
                        <div class="icon">📋</div>
                        <h3>양방향 클립보드</h3>
                        <p>내 PC에서 복사한 텍스트를 원격 PC에 붙여넣거나, 그 반대의 작업도 자유롭게 수행하세요.</p>
                    </div>
                    <div class="card feature-card">
                        <div class="icon">🔊</div>
                        <h3>실시간 사운드</h3>
                        <p>원격 PC에서 재생되는 사운드를 내 PC에서 실시간으로 들으며 작업할 수 있습니다.</p>
                    </div>
                    <div class="card feature-card">
                        <div class="icon">📁</div>
                        <h3>파일 전송</h3>
                        <p>간단한 드래그 앤 드롭(예정)이나 메뉴를 통해 원격 PC와 파일을 손쉽게 주고받을 수 있습니다.</p>
                    </div>
                    <div class="card feature-card">
                        <div class="icon">📊</div>
                        <h3>시스템 모니터링</h3>
                        <p>CPU, 메모리 사용량, 디스크 공간 등 원격 PC의 핵심 시스템 정보를 실시간으로 확인합니다.</p>
                    </div>
                </div>
            </div>
        </section>
            ...
            <div class="container">
                ...
                <div class="grid">
                    <div class="card">
                        ...
                        <h3>연결 설정</h3>
                        <p>클라이언트 프로그램을 실행하고, 제어 PC(매니저)의 IP 주소를 입력합니다. 식별하기 쉬운 '이 PC 이름'을 설정한 후 '연결 시작' 버튼을 누르세요.</p>
                    </div>
                    <div class="card">
                        <div class="step-number">02</div>
                        <h3>백그라운드 실행</h3>
                        <p>연결이 시작되면 프로그램 창은 자동으로 사라지고, 작업 표시줄 트레이 아이콘으로 최소화됩니다. 이제 제어 PC에서 원격 관리를 시작할 수 있습니다.</p>
                    </div>
                    <div class="card">
                        <div class="step-number">03</div>
                        <h3>제어권 관리</h3>
                        <p>여러 매니저가 동시에 접속한 경우, 오직 한 명의 매니저만 '제어 모드'로 전환하여 PC를 조작할 수 있습니다. 이는 입력 충돌을 방지하기 위한 기능입니다.</p>
                    </div>
                </div>
            </div>
        </section>
                ...
                <div class="faq-container">
                    <div class="faq-item">
                        ...
                        <div class="faq-answer">
                            <p>A. 프로그램 실행에 필요한 시스템 드라이버가 설치되지 않았거나 다른 문제일 수 있습니다. <strong>문제를 직접 해결하려고 시도하지 마시고, 즉시 시스템 관리자에게 문의하여 지원을 받으시기 바랍니다.</strong></p>
                        </div>
                    </div>
                    <div class="faq-item">
                        <div class="faq-question">Q. 연결이 되지 않거나 자꾸 끊어집니다.</div>
                        <div class="faq-answer">
                            <p>A. 먼저 제어 PC(매니저)의 IP 주소가 정확한지 확인해주세요. 또한, 클라이언트 PC와 제어 PC가 동일한 네트워크에 있는지, 방화벽이 포트 443을 차단하고 있지는 않은지 확인해야 합니다. 회사나 공용 네트워크의 경우, 네트워크 보안 정책에 의해 연결이 제한될 수 있습니다.</p>
                        </div>
                    </div>
                    <div class="faq-item">
                        <div class="faq-question">Q. 제어권은 어떻게 얻나요?</div>
                        <div class="faq-answer">
                            <p>A. 제어권은 제어 PC(매니저) 프로그램에서 설정할 수 있습니다. 여러 클라이언트 화면 중 제어하고 싶은 PC를 선택하고 '제어 모드'로 전환하면 해당 PC의 제어권을 획득하게 됩니다. 동시에 두 명 이상의 매니저가 한 PC를 제어할 수는 없습니다.</p>
                        </div>
                    </div>
                </div>
            </div>
...
<head>
    ...
    <style>
        ...
        :root {
            --bg-color: #0a0e27; --frame-bg: #1a1f3a; --primary-neon: #64b5f6;
            --secondary-neon: #4dffaf; --text-color: #e0e0e0; --text-dark: #a0a0a0;
            --border-color: #2a3f7f;
        }
        body {
            font-family: 'Poppins', 'Noto Sans KR', sans-serif; background-color: var(--bg-color);
            color: var(--text-color); line-height: 1.8;
        }
        .upload-container {
            padding-top: 120px; min-height: 100vh; display: flex; flex-direction: column;
            align-items: center;
        }
        .section-title {
            font-size: 42px; font-weight: 700; text-align: center; margin-bottom: 60px;
            color: #fff; text-shadow: 0 0 8px rgba(100, 181, 246, 0.5);
        }
        .upload-form {
            background: var(--frame-bg); padding: 40px; border-radius: 10px;
            border: 1px solid var(--border-color); width: 100%; max-width: 500px;
        }
        .upload-form input[type="file"] {
            border: 2px dashed var(--border-color); padding: 20px; width: 100%;
            border-radius: 5px; margin-bottom: 20px; color: var(--text-dark);
        }
        .btn {
            display: inline-block; padding: 15px 35px; background: var(--primary-neon);
            color: var(--bg-color); font-weight: 700; text-decoration: none;
        ...
        .flash-message {
            padding: 15px; margin-bottom: 20px; border-radius: 5px; width: 100%;
            max-width: 500px; text-align: center;
        }
        .flash-success {
            background-color: #1e4620; color: #a7d7a9; border: 1px solid #3c8d40;
        }
        .flash-error {
            background-color: #4a1c1c; color: #f1b0b0; border: 1px solid #c53030;
        }
    </style>
</head>
<body>
...
def index():
    """메인 웹페이지를 렌더링합니다."""
    return render_template_string(HTML_TEMPLATE, program_filename=PROGRAM_FILENAME)
...
def upload_page():
    ...
    if request.method == 'POST':
        ...
        if file and allowed_file(file.filename):
            # werkzeug.utils.secure_filename은 한글 파일명을 제거하므로,
            # 이미 allowed_file에서 파일명을 검증했기 때문에 그대로 사용합니다.
...
if __name__ == '__main__':
    ...
    # 로컬 테스트 환경에서만 app.run()을 사용합니다.
    # 실제 배포 시에는 이 부분이 실행되지 않습니다.
